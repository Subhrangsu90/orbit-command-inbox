...
const SHORTCUT_DEFS = [
  { key: "settingsShortcut", label: "Go to Settings", desc: "Global hotkey to jump here", default: "Alt+s" },
  { key: "shortcutNextEmail", label: "Inbox: Next Email", desc: "Select next email in list", default: "j" },
  { key: "shortcutPrevEmail", label: "Inbox: Previous Email", desc: "Select previous email in list", default: "k" },
  { key: "shortcutReadEmail", label: "Inbox: Open Email", desc: "Open details of selected email", default: "Enter" },
  { key: "shortcutTrashEmail", label: "Inbox: Trash/Delete", desc: "Move selected email to trash", default: "e" },
  { key: "shortcutStarEmail", label: "Inbox: Star Email", desc: "Toggle star on selected email", default: "s" },
  { key: "shortcutComposeEmail", label: "Inbox: Compose", desc: "Open compose dialog", default: "c" },
  { key: "shortcutRefreshEmail", label: "Inbox: Refresh", desc: "Sync latest emails", default: "r" },
  { key: "shortcutCalendarPrev", label: "Calendar: Prev Range", desc: "Move calendar date range back", default: "j" },
  { key: "shortcutCalendarNext", label: "Calendar: Next Range", desc: "Move calendar date range forward", default: "k" },
  { key: "shortcutCalendarToday", label: "Calendar: Today", desc: "Go to today's date", default: "t" },
  { key: "shortcutCalendarCreate", label: "Calendar: Create Event", desc: "Open create event dialog", default: "c" },
  { key: "shortcutCalendarRefresh", label: "Calendar: Refresh", desc: "Refetch Google Calendar events", default: "r" },
] as const;
...
export default function SettingsPage() {
  ...
  const handleRecordKeyDown = (e: React.KeyboardEvent, keyName: keyof WorkspacePreferences) => {
    e.preventDefault();
    if (e.key === "Escape") {
    ...
    if (keys.length > 0) {
      ...
      const reservedKeys = [
        "ctrl+c", "ctrl+v", "ctrl+a", "ctrl+x", "ctrl+z", "ctrl+y", "ctrl+f", "ctrl+p", "ctrl+s"
      ];
      if (reservedKeys.includes(lowerStr)) {
        setValidationError(`Cannot bind to "${shortcutStr}" because it is a reserved system/browser shortcut.`);
        ...
        <div className="transition-all duration-300">
          {activeTab === "general" && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
              {/* Profile Card and Keyboard Shortcuts Column */}
              <div className="space-y-8">
                ...
                {/* Customizable Keyboard Shortcuts Panel */}
                <Card className="bg-surface-container border border-outline-variant p-6 rounded-2xl shadow-sm flex flex-col justify-between min-h-[300px]">
                  <div className="space-y-4">
                    <div className="flex items-center justify-between border-b border-outline-variant pb-3 mb-2">
                      ...
                      {recordingKey && (
                        <span className="text-3xs text-primary font-medium animate-pulse bg-primary/10 border border-primary/20 px-2 py-0.5 rounded-full">
                    ...
                    {validationError && (
                      <div className="bg-error/10 border border-error/20 text-error px-3 py-2 rounded-xl text-3xs font-medium flex items-center gap-1.5">
                    ...
                    <div className="space-y-3 max-h-[360px] overflow-y-auto pr-1">
                      {SHORTCUT_DEFS.map((def) => {
                        const val = preferences[def.key] || def.default;
                        const isThisRecording = recordingKey === def.key;
                        return (
                          <div key={def.key} className="flex items-center justify-between gap-4 py-2 border-b border-outline-variant/10 last:border-b-0">
                            <div className="flex-1 text-left min-w-0">
                              <p className="text-2xs font-semibold text-on-surface truncate">{def.label}</p>
                              <p className="text-[10px] text-on-surface-variant leading-tight truncate">{def.desc}</p>
                            </div>
                            
                            <div className="flex items-center gap-2 shrink-0">
                              <button
                                type="button"
                                onClick={() => setRecordingKey(isThisRecording ? null : def.key)}
                                onKeyDown={(e) => handleRecordKeyDown(e, def.key)}
                                className={`w-28 h-8 px-2 text-[10px] font-mono font-bold rounded-lg border flex items-center justify-center transition-all ${
                                  isThisRecording
                                    ? "bg-primary/20 border-primary text-primary animate-pulse"
                                    : "bg-surface-container-low border-outline-variant text-on-surface hover:bg-surface-container-high cursor-pointer"
                                }`}
                              >
                              ...
                              <button
                                ...
                                className={`p-1.5 rounded-lg border flex items-center justify-center transition-all ${
                                  val === def.default
                                    ? "border-transparent text-on-surface-variant/20 cursor-not-allowed"
                                    : "bg-surface-container-low border-outline-variant text-on-surface hover:text-error hover:bg-error/10 hover:border-error/20 cursor-pointer"
                                }`}
                              >
